...
def download_csv_raw(
    ...
    driver.find_element_by_xpath(
        """ //div[@id='bchh-loginblock']/button[@id='btnLogin'] """
    ).click()
    ...
    sleep(1)
    driver.find_element_by_xpath(
        " //div[@class='formButtons-wrapper']/input[@name='btnSubmit'] "
    ).click()
    ...
    driver.find_element_by_xpath(
        " //div[@class='extra']/ul/li/a[@class='icon billing_export'] "
    ).click()
    sleep(1)
    driver.execute_script(" window.scrollTo(0, 500) ")
    driver.find_element_by_xpath(
        " //input[@type='radio' and @id='data-export-consumption-history'] "
    ).click()
    sleep(1)
    driver.find_element_by_xpath(
        " //input[@type='radio' and @id ='export-format-csv'] "
    ).click
    sleep(1)
    ...
    sleep(1)
    select = Select(
        driver.find_element_by_xpath(" //select[@class='ui-datepicker-month'] ")
    )
    sleep(1)
    select.select_by_visible_text(from_date_month)
    sleep(1)
    select = Select(
        driver.find_element_by_xpath(" //select[@class='ui-datepicker-year'] ")
    )
    ...
    if from_date_day[0] == "0":
        ...
        driver.find_element_by_xpath(
            f" //a[@class='ui-state-default' and text()='{from_date_day}'] "
        ).click()
    else:
        driver.find_element_by_xpath(
            f" //a[@class='ui-state-default' and text()='{from_date_day}'] "
    ...
    if to_date_day[0] == "0":
        ...
        driver.find_element_by_xpath(
            f" //a[@class='ui-state-default' and text()='{to_date_day}'] "
        ).click()
    else:
        driver.find_element_by_xpath(
            f" //a[@class='ui-state-default' and text()='{to_date_day}'] "
    ...
    while timeout < 10:
        ...
        if len(files) > 0:
            ...
            if latest_file.startswith(keyword):
                ...
                )  # Scroll up then click log out before closing down
                sleep(1)
