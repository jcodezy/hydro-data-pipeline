...
def csv_cleaner():
    ...
    df = pd.read_csv(
        csv_to_clean, usecols=["Interval Start Date/Time", "Net Consumption (kWh)"]
    )  # deletes account number & PII
    yesterday = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
    df = df[df["Interval Start Date/Time"].str.startswith(f"{yesterday}")].reset_index(
        drop=True
    )
    df["Interval Start Date/Time"] = pd.to_datetime(
        df["Interval Start Date/Time"],
