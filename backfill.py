...
from_date_day, from_date_month, from_date_year = (
    ...
    "Aug",
    "2020",
)  # date when hydro started
to_date_day, to_date_month, to_date_year = (
    "10",
    "Jan",
    "2021",
)  # daily DAG tasks started on Jan 11th 2021
...
def clean_historical_dataset(
    ...
    if historical_dataset_to_clean:
        ...
        df = df.rename(
            columns={
                "Interval Start Date/Time": "interval_start_date_time",
                "Net Consumption (kWh)": "net_consumption_kwh",
            }
        )
        df["interval_start_date_time"] = pd.to_datetime(df["interval_start_date_time"])

        if df["net_consumption_kwh"].dtype not in [
            "float64",
            "float",
        ]:  # some kwh are "n/a"
            try:
                df["net_consumption_kwh"].fillna(
                    df["net_consumption_kwh"].mean(), inplace=True
                )
        ...
        from_date_month, to_date_month = (
            "0" + str(strptime(from_date_month, "%b").tm_mon),
            "0" + str(strptime(to_date_month, "%b").tm_mon),
        )
        ...
        df.to_parquet(
            f"{DATA_DOWNLOAD_FILEPATH}/{parquet_file_title}.parquet", index=False
        )
        print(
            f"Sucessfully cleaned csv and converted to parquet: {parquet_file_title}.parquet"
        )
...
def upload_historical_data_to_gcs(
    destination_bucket, source_file_name, destination_blob_name
):
